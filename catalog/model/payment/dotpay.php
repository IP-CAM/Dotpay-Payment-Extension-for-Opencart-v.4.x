<?php
namespace Opencart\Catalog\Model\Extension\Dotpay\Payment;

class Dotpay extends \Opencart\System\Engine\Model
{
	public function getMethods(array $address): array
	{
		...
		if (!$this->config->get('config_checkout_payment_address')) {
			$status = true;
		} elseif (!$this->config->get('payment_dotpay_geo_zone_id')) {
			$status = true;
		} else {
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "zone_to_geo_zone` WHERE `geo_zone_id` = '" . (int) $this->config->get('payment_dotpay_geo_zone_id') . "' AND `country_id` = '" . (int) $address['country_id'] . "' AND (`zone_id` = '" . (int) $address['zone_id'] . "' OR `zone_id` = '0')");
		...
		if ($status) {
			$option_data['dotpay'] = [
				'code' => 'dotpay.dotpay',
				'name' => $this->config->get('payment_dotpay_display_name')
			];
			...
			$method_data = array(
				...
				'name' => $this->config->get('payment_dotpay_display_name'),
				'option' => $option_data,
				'sort_order' => $this->config->get('payment_dotpay_sort_order')
			);
		}
